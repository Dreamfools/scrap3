...
#[derive(Clone)]
pub struct GridMath<const MICROCELLS: u64 = 1000> {
    rect: Rect<Screen>,
...
impl<const MICROCELLS: u64> GridMath<MICROCELLS> {
    pub fn new(rect: Rect<Screen>, cols: usize, rows: usize, swap_y: bool) -> Self {
    ...
    /// Returns the closest micro position to the given screen position
    pub fn screen_to_micropos(&self, pos: Vector2<Screen>) -> GridMicroPos {
    ...
    pub fn rect_at_index(&self, index: CellIndex) -> Rect<Screen> {
    ...
    pub fn center_at_index(&self, index: CellIndex) -> Point2<Screen> {
        let (x, y) = self.delinearize(index.0);
    ...
    pub fn unit_cell(&self) -> Rect<Screen> {
